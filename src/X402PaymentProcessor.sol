...
contract X402PaymentProcessor is AccessControl {
    ...
    function registerEarningsStream(uint256 percentage) external {
        ...
        streams[msg.sender] = EarningsStream({
            percentage: percentage, totalEarnings: 0, totalRepaid: 0, lastEarningsUpdate: block.timestamp, active: true
        });
    ...
    function reportEarnings(address agent, uint256 amount, bytes calldata proof) external onlyRole(PROCESSOR_ROLE) {
    ...
    function createPaymentIntent(address agent, uint256 amount, uint256 deadline, bytes32 metadata)
        external
    ...
    // ============ BATCH PROCESSING ============
    function batchProcessRepayments(address[] calldata agents, uint256[] calldata amounts)
        external
        onlyRole(PROCESSOR_ROLE)
    {
