...
contract AIPerformanceOracle is AccessControl, Pausable, AggregatorV3Interface {
    ...
    // ============ METRICS REPORTING ============
    function reportMetrics(address agent, AgentMetrics calldata metrics)
        external
        ...
        whenNotPaused
    {
        ...
        // Add to pending
        pendingReports[agent].push(
            MetricReport({
                reporter: msg.sender, metrics: metrics, timestamp: block.timestamp, stake: reporterStake[msg.sender]
            })
        );
    ...
    function _calculateCompositeScore(AgentMetrics memory m) internal pure returns (uint256) {
        ...
        uint256 responseScore =
            m.responseTime < 60 ? 10000 : (m.responseTime < 300 ? 8000 : (m.responseTime < 900 ? 5000 : 2000));
    ...
    // ============ SLASHING & REWARDS ============
    function _slashOutliers(address agent, MetricReport[] storage reports, uint256 consensusValue) internal {
        for (uint256 i = 0; i < reports.length; i++) {
    ...
    function _rewardReporters(address agent, MetricReport[] storage reports, uint256 totalStake) internal {
        for (uint256 i = 0; i < reports.length; i++) {
    ...
    // ============ MATH HELPERS ============
    function _weightedMedian(uint256[] memory values, uint256[] memory weights, uint256 totalWeight)
        internal
        pure
        returns (uint256)
    {
    ...
    function _weightedAverage(uint256[] memory values, uint256[] memory weights, uint256 totalWeight)
        internal
        pure
        returns (uint256)
    {
