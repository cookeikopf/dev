...
contract ClawCreditUltimateV2 is AccessControl, Pausable, ReentrancyGuard {
    ...
    uint256 public constant FLASH_CALLBACK_SUCCESS = uint256(keccak256("ClawCreditFlashBorrower.onFlashLoan"));
    ...
    event FlashLoanExecuted(address indexed caller, address indexed receiver, uint256 amount, uint256 fee);
    ...
    constructor(
        ...
        if (
            admin == address(0) || guardian == address(0) || treasury == address(0) || x402Operator == address(0)
                || usdcToken == address(0) || reputation == address(0) || usdcFeed == address(0) || aiFeed == address(0)
    ...
    function deposit(uint256 assets) external nonReentrant whenNotPaused returns (uint256 sharesMinted) {
    ...
    function withdraw(uint256 assets) external nonReentrant whenNotPaused returns (uint256 sharesBurned) {
        if (assets == 0) revert InvalidAmount();
        ...
        returns (bool)
    {
        ...
        bytes32 callbackResult = IClawFlashBorrower(receiver).onFlashLoan(msg.sender, address(usdc), amount, fee, data);
    ...
    function setRiskConfig(
        ...
        if (
            newLiquidationThresholdBps < 7_000 || newLiquidationThresholdBps > 9_500 || newMinAiScoreBps > BPS
                || newMinUsdcPegBps > BPS || newGracePeriod > 14 days || newOracleHeartbeat > 24 hours
        ) revert InvalidAmount();
    ...
    function _applyRepayment(
        ...
        emit LoanRepayment(
            loanId, payer, loan.borrower, principalPaid + interestPaid, principalPaid, interestPaid, remainingDebt
        );

        if (autoRepay) {
            emit AutoRepaymentProcessed(
                loanId, loan.borrower, grossEarnings, principalPaid + interestPaid, loan.autoRepayBps
            );
        }
    ...
    function _tryReadAiScore() internal view returns (bool ok, uint256 scoreBps) {
        try aiPerformanceFeed.latestRoundData() returns (uint80, int256 answer, uint256, uint256 updatedAt, uint80) {
