...
contract ClawCreditAgentStandardV3 is AccessControl, Pausable, ReentrancyGuard, EIP712, IAgentStandard {
    ...
    event TrancheWithdrawn(
        address indexed lender, uint8 indexed tranche, uint256 assetsOut, uint256 yieldOut, uint256 sharesBurned
    );
    ...
    event TaskCreated(
        uint256 indexed taskId, address indexed agent, uint256 receivable, uint256 dueDate, bool escrowed
    );
    ...
    event AttestationSubmitted(
        address indexed agent, address indexed attestor, uint256 scoreBps, uint256 confidenceBps, bytes32 evidence
    );
    event PassportUpdated(
        address indexed agent, uint256 indexed sourceChainId, uint256 scoreBps, uint256 confidenceBps, uint256 nonce
    );
    ...
    event IntentExecuted(
        uint256 indexed loanId, address indexed caller, address indexed to, uint256 amount, bytes32 intentHash
    );
    ...
    event CovenantReported(
        uint256 indexed loanId, int256 pnlBps, uint256 drawdownBps, uint256 completionBps, bool breached
    );
    ...
    constructor(
        ...
        if (
            admin == address(0) || guardian == address(0) || treasury == address(0) || usdcToken == address(0)
                || reputation == address(0) || usdcFeed == address(0) || aiFeed == address(0)
    ...
    function stakeReinsurance(uint256 assets) external nonReentrant whenNotPaused returns (uint256 sharesMinted) {
        if (assets == 0) revert InvalidAmount();

        uint256 poolBefore = reinsurancePool;
        sharesMinted = totalReinsuranceShares == 0 ? assets : (assets * totalReinsuranceShares) / poolBefore;
        if (sharesMinted == 0) revert InvalidAmount();
    ...
    function unstakeReinsurance(uint256 sharesBurned) external nonReentrant whenNotPaused returns (uint256 assetsOut) {
    ...
    function verifySocialAccount(
        ...
        if (!(hasRole(ORACLE_MANAGER_ROLE, verifier) || hasRole(VERIFIER_ROLE, verifier))) {
            revert DelegationUnavailable();
        }
        ...
        returns (uint16 underwriterId)
    {
        ...
        underwriters[underwriterId] = Underwriter({
            model: model, feeBps: uint16(feeBps), active: true, loansQuoted: 0, defaults: 0, accruedFees: 0
        });
    ...
    function openRevolvingLoan(
        ...
        loanId = _openLoan(msg.sender, address(0), amount, tenorDays, underwriterId, intentHash, covenant, 0, 0, false);
    }
    ...
    function openTaskBackedLoan(
        ...
        loanId = _openLoan(
            msg.sender, address(0), amount, tenorDays, underwriterId, intentHash, covenant, taskId, coverageBps, false
        );
    }
    ...
    function openDelegatedLoan(
        ...
        loanId = _openLoan(msg.sender, sponsor, amount, tenorDays, underwriterId, intentHash, covenant, 0, 0, true);
    }
    ...
    function streamRepay(uint256 loanId, uint256 amount, bytes32 streamRef) external nonReentrant whenNotPaused {
        if (amount == 0) revert InvalidAmount();
        Loan storage loan = loans[loanId];
        if (!loan.active) revert LoanNotActive();
    ...
    function submitCovenantReport(uint256 loanId, int256 pnlBps, uint256 drawdownBps, uint256 completionBps)
        external
        onlyRole(ATTESTOR_ROLE)
    {
        Loan storage loan = loans[loanId];
        ...
        covenantReports[loanId] = CovenantReport({
            pnlBps: pnlBps, drawdownBps: drawdownBps, completionBps: completionBps, reportedAt: uint40(block.timestamp)
        });
    ...
    function withdrawProtocolFees(address recipient, uint256 amount) external nonReentrant onlyRole(TREASURY_ROLE) {
        if (recipient == address(0)) revert InvalidAddress();
        uint256 withdrawable = protocolFees > underwriterFeeLiability ? protocolFees - underwriterFeeLiability : 0;
    ...
    function setCoreConfig(
        ...
        if (
            newMinLoan == 0 || newMaxExposurePerAgent > 500e6 || newMaxTenorDays == 0 || newMaxTenorDays > 60
                || newGracePeriod > 14 days || newLiquidationThresholdBps < 6_500 || newLiquidationThresholdBps > 9_500
    ...
    function _openLoan(
        ...
        if (amount > type(uint96).max || tenorDays == 0 || tenorDays > maxTenorDays) {
            revert InvalidAmount();
        }
    ...
    function _quoteUnderwriter(
        ...
        try IAgentUnderwriterModel(uw.model).quote(borrower, amount, tenorDays, score, taskCoverageBps) returns (
            uint256 modelApr, uint256 confidenceBps, uint256 maxAmount
        ) {
