...
contract ERC8004ReputationRegistry is AccessControl, Pausable {
    ...
    function recordProtocolInteraction(address agent, address protocol) external {
        ...
        bool exists = false;
        for (uint i = 0; i < rep.protocolsUsed.length; i++) {
    ...
    function addSocialProof(
        ...
        socialProofs[agent].push(SocialProof({
            platform: platform,
            username: username,
            followers: followers,
    ...
    function _calculateScore(address agent) internal view returns (uint256) {
        ...
        uint256 repaymentScore = totalLoans > 0 
            ? (rep.successfulRepayments * 10000) / totalLoans 
            : 5000;
