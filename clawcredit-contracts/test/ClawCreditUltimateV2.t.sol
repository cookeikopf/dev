...
contract ClawCreditUltimateV2Test is Test {
    ...
    function test_RefreshCreditLineUsesReputationAndAi() public {
        ...
        (uint96 limit, uint96 used, uint16 aprBps, uint40 lastUpdated, bool active) =
            pool.creditLines(agentHigh);
        assertTrue(active);
    ...
    function test_MaxExposureCappedAt500USDC() public {
        _draw(agentHigh, 300e6, 0, 30);
        _draw(agentHigh, 200e6, 0, 30);

        vm.prank(agentHigh);
        vm.expectRevert(ClawCreditUltimateV2.BorrowCapExceeded.selector);
        pool.drawCreditLine(1e6, 0, 30);
    }
    ...
    function _draw(address borrower, uint256 amount, uint256 collateral, uint32 tenorDays)
        internal
        returns (uint256)
    {
        vm.prank(borrower);
