...
contract ClawCreditUltimateV3Test is Test {
    ...
    function test_ReputationVoteUpdatesConsensus() public {
        // Setup reporter - need 5 ETH minimum stake (not 6,000 USDC)
        vm.deal(admin, 10 ether);
        vm.prank(admin);
        aiOracle.stakeReporter{value: 6 ether}();
        ...
        assertEq(aiOracle.getConsensusScore(agent), 0);
    }
    ...
    function test_WithdrawalRequiresLiquidity() public {
        ...
        // Try to withdraw more than available (all shares)
        // This should fail because loans are using the liquidity
        uint256 lenderShares = pool.lenderShares(lender);
        assertGt(lenderShares, 0, "Lender should have shares");

        // The withdrawal should revert because not enough liquid funds
        vm.prank(lender);
        vm.expectRevert(ClawCreditUltimateV3.InsufficientLiquidity.selector);
